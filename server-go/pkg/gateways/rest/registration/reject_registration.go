...
// @Success 200 {object} domain.Registration "Rejected registration"
// @Failure 400 "Bad request"
// @Failure 401 "Unauthorized"
// @Failure 403 "Forbidden - only organizer can reject"
// @Failure 404 "Registration not found"
// @Failure 500 "Internal server error"
// @Router /registrations/{event_id}/{user_id}/reject [put]
func (h *Handler) rejectRegistration(c *gin.Context) {
	...
	if event.Type == domain.EventTypeGame {
		...
		// Для других типов событий проверяем права организатора
		if organizer.ID != event.Organizer.ID {
			c.JSON(http.StatusForbidden, gin.H{"error": "only event organizer can reject registrations"})
			return
		}
	}

	// Обновляем статус на CANCELLED
	cancelledStatus := domain.RegistrationStatusCancelled
	updatedReg, err := h.cases.Registration.AdminUpdateRegistrationStatus(c, userID, eventID, cancelledStatus)
	if ginerr.AbortIfErr(c, err, http.StatusInternalServerError, "failed to reject registration") {
		return
	}
