      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-4 lg:gap-6">
        ...
        <Card className="bg-zinc-900 border-zinc-800">
          <CardHeader className="pb-4">
            <CardTitle className="text-white flex items-center justify-between text-lg">
              {isCreating ? 'Создать программу' : editingId ? 'Редактировать программу' : 'Форма управления'}
            ...
            {isCreating || editingId ? (
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                  ...
                  <Textarea
                    ...
                    className="bg-zinc-700 border-zinc-600 text-zinc-500 placeholder:text-zinc-500 min-h-[60px] md:min-h-[80px] mt-1 resize-none cursor-not-allowed"
              ...
              <div className="text-center py-6 md:py-8">
                <p className="text-zinc-400 mb-4 text-sm md:text-base">
                  {canEdit 
                    ? "Выберите программу для редактирования или создайте новую"
                    : "Только суперпользователи могут управлять программами лояльности"
                  }
                </p>
                {canEdit && (
                  <Button
        ...
        <Card className="bg-zinc-900 border-zinc-800">
          ...
          <CardContent className="pt-0">
            {loyalties.length === 0 ? (
              <div className="text-center py-6 md:py-8">
                <p className="text-zinc-400 text-sm md:text-base">Нет программ лояльности</p>
              </div>
            ) : (
                      ...
                      <div className="flex items-start justify-between gap-3">
                        <div className="flex-1 min-w-0">
                          <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-2 mb-1 md:mb-2">
                            <h3 className="text-white font-medium text-base md:text-lg truncate">{loyalty.name}</h3>
                            <Badge variant="outline" className="border-green-600 text-green-300 text-xs w-fit">
                              {loyalty.discount}% скидка
                            </Badge>
                          </div>
                          
                          {loyalty.description && (
                            <p className="text-zinc-400 text-xs md:text-sm line-clamp-2 md:line-clamp-3">{loyalty.description}</p>
                          )}
                        </div>
