...
export default function UserCard({ user, onClick }: UserCardProps) {
  return (
    <div 
      className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 mb-4 cursor-pointer hover:shadow-md transition-all duration-200"
      onClick={onClick}
    >
      <div className="flex items-start">
        <div className="flex-shrink-0 mr-4 relative flex flex-col items-center">
          {user.avatar ? (
            <img 
              src={user.avatar} 
      ...
      {user.bio && (
        <div className="mt-4 text-gray-600 text-sm leading-relaxed overflow-hidden" style={{
          display: '-webkit-box',
          WebkitLineClamp: 3,
