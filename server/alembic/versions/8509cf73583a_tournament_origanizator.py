...
def upgrade() -> None:
    ...
    first_user_id = connection.execute(
        text("SELECT id FROM users ORDER BY telegram_id LIMIT 1")
    ).scalar()
    ...
    op.create_foreign_key(
        "fk_tournaments_organizator_id_users",
        "tournaments",
        "users",
        ["organizator_id"],
        ["id"],
...
def downgrade() -> None:
    ...
    if column_exists:
        ...
        for constraint in constraints:
            ...
            connection.execute(
                text(
                    f'ALTER TABLE tournaments DROP CONSTRAINT IF EXISTS "{constraint_name}"'
                )
            )

        # Now drop the column
        connection.execute(
            text("ALTER TABLE tournaments DROP COLUMN IF EXISTS organizator_id")
        )
