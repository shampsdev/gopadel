...
async def get_tournaments(
    ...
    tournaments = tournament_crud.get_tournaments_with_participants(db, user.rank, available)
    return tournaments


@router.get("/{tournament_id}", response_model=TournamentResponse)
async def get_tournament(db: SessionDep, tournament_id: UUID):
    tournament = tournament_crud.get_tournament_with_participants_by_id(db, tournament_id)
